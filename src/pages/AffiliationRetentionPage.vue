    ...
    <q-card class="q-mb-md shadow-2" flat bordered>
      ...
      <q-card-section>
        <div class="row q-col-gutter-md">
          ...
          <div class="col-12 col-sm-6 col-md-2">
            <q-select v-model="selectedTipoContrato" :options="tipoContratoOptions" label="Tipo de Contrato" outlined
              dense @update:model-value="loadFacturaciones">
              <template v-slot:prepend>
  ...
  setup() {
    ...
    const exportToExcel = async () => {
      try {
        ...
        // Always send tipo_contrato to avoid including FACTURACION
        // If "Todos" is selected, send the first type (we'll handle this differently)
        if (selectedTipoContrato.value?.value) {
          params.tipo_contrato = selectedTipoContrato.value.value
        } else {
          // When "Todos" is selected, default to AFILIACION for now
          // TODO: This should ideally make two requests or backend should support multiple types
          params.tipo_contrato = 'AFILIACION,RETENCION'
        }
